...
// Make UART calls to the SIM900. 
// Use this function to expand the GPRS module's functionality by sending AT commands and recieving the SIM900's replies. 
// If you implement something particularly useful, submit a pull request!
// Every time we interact with the SIM900, it's through a series of UART calls and responses. 
// This function makes that less painful. 
// Note that this function requires that the SIM900 be configured to echo the commands it recieves (the default) in order for it to function properly.
/*
Args
  message
...
// Other results, if not of the form [`messages[n]`, 'OK'] error out and pass false to the callback. 
// The arguments `messages` and `patience` must be of the same length. 
// Like `_txrx`, this function is also useful for expanding the module's functionality.
/*
mesages
...
GPRS.prototype.dial = function (number, callback) {
  var one_year = 1000*60*60*24*365; // hang up in a year
  ...
  if(this.inACall) {
    ...
    this.inACall = true;
    this._txrx('ATD' + number + ';', one_year, function (err, data) {
      this.inACall = false;
      callback(err, data);
    });
  }
